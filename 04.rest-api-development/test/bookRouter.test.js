...
describe('Books API', () => {
  ...
  describe('PUT /api/v1/books/:id', () => {
    ...
    it('should return 400 if neither title nor author is provided', async () => {
      ...
      const response = await request(app).put('/api/v1/books/1').send({})

      expect(response.statusCode).toBe(400)
      expect(response.body).toHaveProperty(
        'message',
        'At least one of title or author must needed'
      )
    })
  })

  describe('DELETE /api/v1/books/:id', () => {
    it('should delete the book and return it with status 200', async () => {
      const mockBooks = [
        {
          id: 1,
      ...
      expect(response.body.book).toEqual(mockBooks[0]);
      expect(fs.writeFileSync).toHaveBeenCalledWith(
        expect.any(String),
        JSON.stringify([mockBooks[1]], null, 2)
      )
    })
    ...
    it('should return 404 if book is not found', async () => {
      ...
      const response = await request(app).delete('/api/v1/books/999')
